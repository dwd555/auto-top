...
import json
import numpy
with open("./cpuSetting.txt","r") as fi:
...
def writeData(process,log,index,full,sheet,danwei):#1、写入数据，2、日志，3、需要查看的信息，4、是否全输出,否的话方差>1的才写入,5、是否带单位
	xlsIndex=0
...
if __name__ == '__main__':
	try:
		main(setting["filename"],setting["progress"],'./cpuResult.xls',setting["arg"],bool(setting["full"]),bool(setting['danwei']))
	except Exception as e:
		print(e)
